...
write.csv(trees_data_update_5, paste0(getwd(), out.path, paste(trees_data_update_5$inv[1], "LT_update_6_3", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
alnus_wag <-  trees_data_update_5[trees_data_update_5$compartiment %in% c("w_agb") &
                                    ...
                                    trees_data_update_5$min_org == "min" &
...
# min function: 28 
alnus_wag$paper_ID[alnus_wag$B_kg_tree == min(alnus_wag$B_kg_tree)] # 14
alnus_wag_mean_func$paper_ID[alnus_wag_mean_func$B_kg_tree == min(alnus_wag_mean_func$B_kg_tree)] # 24!!!!

# min mean biomass
min(alnus_wag_mean_func$B_kg_tree) #  231.2137
...
trees_data_al_bet_wag <- (trees_data_update_5[compartiment == "w_agb" & 
                                                bot_name %in% c("Betula pubescens"
                                                                  # spp is being selected too because we assume alnus and betula at organic plots to be glutinosa/ pubescens
                                                                ,"Alnus glutinosa" 
...
write.csv(turkey_out_al_bet, paste0(getwd(), out.path, paste(trees_data_update_5$inv[1], "turkey_output_B_species_min", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
