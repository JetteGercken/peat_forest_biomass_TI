...
trees_data <- read.delim(file = here(paste0(out.path, "HBI_LT_update_4.csv")), sep = ",", dec = ".")
trees_stat_2 <- read.delim(file = here(paste0(out.path, trees_data$inv[1], "_LT_stat_2.csv")), sep = ",", dec = ".") %>% 
...
RG_data <- read.delim(file = here(paste0(out.path, trees_data$inv[1], "_RG_update_4.csv")),sep = ",", dec = ".")
RG_stat_2 <- read.delim(file = here(paste0(out.path, trees_data$inv[1], "_RG_stat_2.csv")), sep = ",", dec = ".") %>% 
  mutate(inv = inv_name(inv_year))
...
DW_data <- read.delim(file = here(paste0(out.path, trees_data$inv[1], "_DW_update_4.csv")), sep = ",", dec = ".")
DW_stat_2 <- read.delim(file = here(paste0(out.path, trees_data$inv[1], "_DW_stat_2.csv")), sep = ",", dec = ".") %>% 
  mutate(inv = inv_name(inv_year))
...
# 6. data export ----------------------------------------------------------
write.csv(LT_summary, paste0(out.path, paste(LT_summary$inv[1], "LT_stocks_ha_all_groups", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
write.csv(RG_summary, paste0(out.path, paste(RG_summary$inv[1], "RG_stocks_ha_all_groups", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
write.csv(DW_summary, paste0(out.path, paste(DW_summary$inv[1], "DW_stocks_ha_all_groups", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
write.csv(LT_RG_DW_TY, paste0(out.path, paste(LT_RG_DW_P$inv[1], "LT_RG_DW_stocks_ha_all_groups_standtype", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
write.csv(LT_RG_DW_P, paste0(out.path, paste(LT_RG_DW_P$inv[1], "LT_RG_DW_stocks_ha_all_groups_plot", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
write.csv(LT_RG_DW, paste0(out.path, paste(LT_RG_DW_P$inv[1], "LT_RG_DW_stocks_ha_all_groups", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
ggplot(data = (LT_summary %>% filter(stand_component == "LT" & 
                                      ...
                                      stand == "all" &
                                      SP_code == "all" &
                                      compartiment %in% c("total")) %>% 
         left_join(.,LT_summary %>% filter(stand_component == "LT" & 
                                             plot_ID != "all" & 
                                             stand == "all" &
                                             SP_code == "all" &
                                             compartiment %in% c("total")) %>% 
                     group_by(stand_component) %>% 
                     summarise(mean_B_tha = mean(B_t_ha)) , by = "stand_component") %>% 
           left_join(., soil_types_db %>% select(bfhnr_2 , min_org), by = c("plot_ID" = "bfhnr_2")) %>% 
           mutate(min_org = ifelse(inv == "momok", "org", min_org))), 
       aes(x = as.factor(plot_ID), y = B_t_ha)) + 
  geom_bar(aes(color = min_org),stat="identity", position=position_dodge(width=0.9))+
...
ggplot(data = (LT_summary %>% filter(stand_component == "LT" & 
         ...
         left_join(.,LT_summary %>% filter(stand_component == "LT" & 
                     ...
                     summarise(mean_B_tha = mean(B_t_ha)) , by = "stand_component")), 
       aes(x = as.factor(plot_ID), y = B_t_ha))+ 
  geom_bar(aes(color = compartiment), stat="identity")+
  geom_line(aes(y = mean_B_tha, group = 1), color = "red")

# carbon by plot by compartiment with mean total biom over all plots
ggplot(data = (LT_summary %>% filter(stand_component == "LT" & 
                                       plot_ID != "all" & 
                                       stand == "all" &
                                       SP_code == "all" &
                 ...
                 left_join(.,LT_summary %>% filter(stand_component == "LT" & 
                                                     ...
                                                     compartiment %in% c("total")) %>% 
                             group_by(stand_component) %>% 
                             summarise(mean_C_tha = mean(C_t_ha)) , by = "stand_component") %>% 
                 left_join(., soil_types_db %>% select(bfhnr_2 , min_org), by = c("plot_ID" = "bfhnr_2")) %>% 
                 mutate(min_org = ifelse(inv == "momok", "org", min_org))), 
       aes(x = as.factor(plot_ID), y = C_t_ha)) + 
  geom_bar(aes(color = min_org),stat="identity", position=position_dodge(width=0.9))+
  geom_line(aes(y = mean_C_tha, group = 1), color = "red")

# boxplot of carbon stock per plot grouped by min vs. org
ggplot(data = (LT_summary %>% filter(stand_component == "LT" & 
                                       plot_ID != "all" & 
                                       stand == "all" &
                                       SP_code == "all" &
                 ...
                 mutate(min_org = ifelse(inv == "momok", "org", min_org)) %>% 
                 left_join(., 
                           LT_summary %>% filter(stand_component == "LT" & 
                                                   plot_ID != "all" & 
                                                   stand == "all" &
                                                   SP_code == "all" &
                                                   compartiment %in% c("total")) %>% 
                             ...
                             mutate(min_org = ifelse(inv == "momok", "org", min_org)) %>% 
                             group_by(min_org) %>% 
                             summarise(mean_C_min_org = mean(C_t_ha)))), 
       aes(x = as.factor(min_org), y = C_t_ha)) + 
  geom_boxplot()+
  geom_point(aes(y = mean_C_min_org, group = min_org), color = "red")


LT_summary_soil <- LT_summary %>% filter(stand_component == "LT" & 
                                           plot_ID != "all" & 
                                           stand == "all" &
                                           SP_code == "all" &
                                           compartiment %in% c("total")) %>% 
