...
trees_removed <- read.delim(file = paste0(out.path, trees_data$inv[1], "_LT_removed.csv"), sep = ",", dec = ".")



# 0.4 data preparation ---------------------------------------------------------
trees_data <- trees_data %>% mutate(H_m_nls = as.numeric(H_m_nls))  %>% distinct() 
...
for (i in 1:nrow(unique(trees_data[, c("plot_ID", "tree_ID")]))) {
  ...
  Ht = na.omit(as.numeric(unique(trees_data$H_m_nls[trees_data$plot_ID==my.plot.id & trees_data$tree_ID==my.tree.id])))
...
trees_removed <- plyr::rbind.fill(trees_removed, 
                                  trees_data  %>% 
                                    ...
                                    mutate(rem_reason = "single LT excluded DBH based during stock calculation"))
...
write.csv(trees_update_4, paste0(out.path, paste(unique(trees_update_4$inv)[1], "LT_update_5", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
stop("notes of 07_LT_stocks based on DBH only start here, no need to run")
# NOTES:  -----------------------------------------------------------------
