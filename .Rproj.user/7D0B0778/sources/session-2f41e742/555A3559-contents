...
try({
  BZE3_trees <- read.delim(file = here(out.path,"BZE3_LT_update_1.csv"), sep = ",", dec = ".")
...
# this statement stops the whole BZE3 related code from running if there are no BZE3 data
# the if-statments before every loop are constructed for the case that there are BZE3 data but not for the respective tree inventory status
if(exists('BZE3_trees') == T){   #important# you can find the end of the statement by searching " #important# "
...
## TEST create fake/ practice BZE3/ post dataset from HBI/ pre data
...
} ## close if statement that blocks whole code if BZE3 data don´t exist #important#
...
write.csv(HBI_trees_removed, paste0(out.path, paste(unique(HBI_trees_update_02$inv)[1], "LT", "removed", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")

if(exists('BZE3_trees') == T){
write.csv(BZE3_trees_update_02,paste0(out.path, paste(unique(BZE3_trees_update_02$inv)[1], "LT", "update","2", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
write.csv(BZE3_trees_removed, paste0(out.path, paste(unique(BZE3_trees_update_02$inv)[1], "LT", "removed", sep = "_"), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
}## close if statement that blocks whole code if BZE3 data don´t exist


stop("this is where LT inventory sorting HBI BZE3 paper ends")
# write.csv2(BZE3_trees, paste0(out.path, paste(unique(BZE3_trees$inv)[1], "LT", "update","0", "demo", sep = "_"), ".csv"))
